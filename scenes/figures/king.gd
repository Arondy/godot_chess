...
func king_check_move(dest: String, attackedMask: Array, forKing: bool) -> bool:
	...
	if (abs(dx) > 1 or abs(dy) > 1 or cellName == dest
			or (not forKing and Tools.board.get_node(dest).has_friendly_figure(color))):
		return false
...
func get_possible_moves(forKing: bool) -> Array:
	...
	for i in range(-1, 2):
		for j in range(-1, 2):
			...
			if king_check_move(move, mask, forKing):
				res.append(move)
